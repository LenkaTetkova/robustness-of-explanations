...
from src.explanations.process_results import read_results
from src.explanations.utils import load_results
...
class PixelFlipping(PerturbationMetric):
    ...
    def evaluate_instance(
        ...
        y_pred_orig = float(model.predict(x_input)[:, y])
        ...
        for i_ix, a_ix in enumerate(a_indices[:: self.features_in_step]):
            ...
            preds[i_ix] = y_pred_perturb/y_pred_orig
...
def load_scores(explainer, augs, folder, file_results):
    scores = []
    for aug in augs:
        n_images = 0
        output = list(os.walk(folder + aug + '/' + explainer + '/'))[0]
        image_ids = output[1]
        all_results = read_results(file_results)
        results = load_results(all_results)
        for id in image_ids:
            if results[id]:
                file_name = folder + aug + '/' + explainer + '/' + id + '/' + 'pixel_flipping.yaml'
                with open(file_name, 'rb') as f:
                    PF_score = pickle.load(f)
                scores.append(PF_score[0])
                n_images += 1
        print(f"{aug}: {n_images} out of {len(image_ids)} images.")
    return scores
...
def plot(scores, path):
    ...
    for key, value in scores.items():
        plt.plot(
                 np.linspace(0, 1, len(value)),
                 value,
                 label=f"{key}",
                 )
    plt.xlabel("Fraction of pixels flipped")
    plt.ylabel("Mean Predictions/the original prediction")
    ...
    plt.legend()
    plt.savefig(path + 'pixel_flipping.png')
    plt.show()
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    parser.add_argument("folder_to_save", type=str,
                        help="Root folder where the figure will be saved.")
    parser.add_argument("file_name_results", type=str,
                        help="Name of the file with results.")
    args = parser.parse_args()

    scores_to_plot = {}
    for explainer in args.explainer:
        scores = load_scores(explainer, args.aug, args.folder, args.file_name_results)
        mean_curve = np.mean(np.vstack(np.array(scores)), axis=0)
        scores_to_plot[explainer] = mean_curve
        ...
        score = np.mean(PF_score)
        SEM = sem(np.vstack(PF_score), axis=0)[0]
        print(f"Explainer {explainer}: pixel flipping AOC score: {score}+-{SEM}.")
    plot(scores_to_plot, args.folder_to_save)
